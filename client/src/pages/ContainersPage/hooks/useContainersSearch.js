/**
 * Hook for managing Containers page search functionality
 */

import { useState, useMemo } from "react";
import { useDebounce } from "../../../hooks/useDebounce";
import { CONTAINERS_IMAGE_SOURCE_FILTERS } from "../../../constants/containersPage";
 ...
 */
export const useContainersSearch = (groupedStacks, selectedImageSourceFilters = new Set()) => {
        ...
        .map((stack) => ({
          ...
          containers: stack.containers.filter((container) => {
            ...
            if (provider === "dockerhub") {
              return selectedImageSourceFilters.has(CONTAINERS_IMAGE_SOURCE_FILTERS.DOCKERHUB);
            } else if (provider === "ghcr" || provider === "github-releases") {
              return selectedImageSourceFilters.has(CONTAINERS_IMAGE_SOURCE_FILTERS.GITHUB);
            } else if (provider === "gitlab") {
              return selectedImageSourceFilters.has(CONTAINERS_IMAGE_SOURCE_FILTERS.GITLAB);
            } else if (provider === "gcr") {
              return selectedImageSourceFilters.has(CONTAINERS_IMAGE_SOURCE_FILTERS.GOOGLE);
            }
