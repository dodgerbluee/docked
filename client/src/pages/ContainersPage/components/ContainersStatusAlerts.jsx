/**
 * Containers status alerts component
 */
 ...
 * @param {boolean} props.pullingSourcesOnly - Whether pulling source data only
 * @param {string} props.localPullError - Local pull error message
 * @param {Function} props.onDismissError - Error dismiss handler
 */
const ContainersStatusAlerts = ({
  pullingDockerHub,
  pullingSourcesOnly,
  localPullError,
  onDismissError,
}) => {
  if (!pullingDockerHub && !pullingSourcesOnly && !localPullError) {
    return null;
  }
      ...
      {pullingSourcesOnly && (
        <Alert variant="info" className={styles.alert}>
          <div className={styles.pullStatusContent}>
            ...
            <div className={styles.pullStatusText}>
              <strong>Updating source data...</strong>
              <span>Fetching container information from sources</span>
            </div>
          </div>
        </Alert>
      )}
      {!pullingDockerHub && !pullingSourcesOnly && localPullError && (
...
ContainersStatusAlerts.propTypes = {
  pullingDockerHub: PropTypes.bool.isRequired,
  pullingSourcesOnly: PropTypes.bool.isRequired,
  localPullError: PropTypes.string.isRequired,
  onDismissError: PropTypes.func.isRequired,
};

export default ContainersStatusAlerts;
