  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.error || "Failed to save Docker Hub credentials");
      }
    } catch (err) {
      setError(
        err.response?.data?.error || "Failed to save Docker Hub credentials. Please try again."
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <Input
          ...
          placeholder={existingCredentials ? "Leave blank to keep current token" : "dckr_pat_..."}
          disabled={loading}
          helperText={
            existingCredentials
