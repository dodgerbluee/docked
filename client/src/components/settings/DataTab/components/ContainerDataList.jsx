...
const ContainerDataList = forwardRef(
  (
    ...
    const correlatedRecords =
      dataEntries.length > 0 && dataEntries[0]?.correlatedRecords
    ...
    if (
      ...
      (!rawDatabaseRecords || Object.keys(rawDatabaseRecords).length === 0)
    ) {
    ...
    return (
      <div className={styles.dataWrapper}>
        ...
        <div className={styles.viewModeToggle}>
          <button
            className={`${styles.viewModeButton} ${viewMode === "formatted" ? styles.active : ""}`}
            onClick={() => handleViewModeChange("formatted")}
          >
            Formatted View
          </button>
          <button
            className={`${styles.viewModeButton} ${viewMode === "raw" ? styles.active : ""}`}
