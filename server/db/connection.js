        ...
        FOREIGN KEY (tracked_app_id) REFERENCES tracked_apps(id) ON DELETE SET NULL
          ...
          (err) => {
            if (err) {
              ...
              db.get("SELECT sql FROM sqlite_master WHERE type='table' AND name='tracked_app_upgrade_history'", (err, row) => {
                if (!err && row && row.sql) {
                  // Check if the constraint is missing 'docker'
                  if (row.sql.includes("CHECK(provider IN ('github', 'gitlab'))") || 
                      row.sql.includes("CHECK (provider IN ('github', 'gitlab'))")) {
                    logger.info("Migrating tracked_app_upgrade_history table to add 'docker' provider support");
                    
                    // Recreate the table with updated constraint
                    db.run("BEGIN TRANSACTION", (txErr) => {
                      if (txErr) {
                        logger.error("Error starting transaction for tracked_app_upgrade_history migration:", { error: txErr });
                        return;
                      }
                      
                      // Create temporary table with new schema
                      db.run(`CREATE TABLE tracked_app_upgrade_history_new (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        user_id INTEGER NOT NULL,
                        tracked_app_id INTEGER,
                        app_name TEXT NOT NULL,
                        ...
                        if (createErr) {
                          logger.error("Error creating new tracked_app_upgrade_history table:", { error: createErr });
                          db.run("ROLLBACK");
                          return;
                        }
                        
                        // Copy data from old table
                        db.run(`INSERT INTO tracked_app_upgrade_history_new SELECT * FROM tracked_app_upgrade_history`, (copyErr) => {
                          if (copyErr) {
                            logger.error("Error copying tracked_app_upgrade_history data:", { error: copyErr });
                            db.run("ROLLBACK");
                            return;
                          }
                          ...
                          db.run("DROP TABLE tracked_app_upgrade_history", (dropErr) => {
                            if (dropErr) {
                              logger.error("Error dropping old tracked_app_upgrade_history table:", { error: dropErr });
                              db.run("ROLLBACK");
                              return;
                            }
                            
                            // Rename new table
                            db.run("ALTER TABLE tracked_app_upgrade_history_new RENAME TO tracked_app_upgrade_history", (renameErr) => {
                              if (renameErr) {
                                logger.error("Error renaming tracked_app_upgrade_history table:", { error: renameErr });
                                db.run("ROLLBACK");
                                return;
                              }
                              ...
                              db.run("COMMIT", (commitErr) => {
                                if (commitErr) {
                                  logger.error("Error committing tracked_app_upgrade_history migration:", { error: commitErr });
                                } else {
                                  logger.info("Successfully migrated tracked_app_upgrade_history table");
                                }
                              });
                            });
                          });
              ...
              db.run(
                ...
                (idxErr) => {
                  if (idxErr && !idxErr.message.includes("already exists")) {
                    logger.error("Error creating tracked_app_upgrade_history tracked_app_id index:", {
                      error: idxErr,
                    });
                  }
                }
